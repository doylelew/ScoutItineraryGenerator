...
#permanet back and next buttons for navigating frame
...
class LocationUnit(SubFrame):
	...
	def render(self):
		...
		self.overide_checkbox=ttk.Checkbutton(self.full_time_frame, variable=self.checkbox_value, onvalue='disabled', offvalue='!disabled' )
...
class ListLocationsScreen(SubFrame):
	...
	def render(self):
		...
		if self.excel_file.dataframeIsValid():
			...
			#add all to cleaner
			self.cleaner = [self.file_name_label, self.column_label, self.choose_column, self.sumbit_options, self.horizontal_seperator1,self.horizontal_seperator2, self.number_label, self.time_label, self.number_of_locations, self.minutes_per]


		else:
			self.file_name_label = ttk.Label(self, text=f"'{str(self.excel_file)}' is not a file,\nplease hit 'back' and try again")
	...
	def submitOptions(self):
		...
		if not self.number_of_locations.get().isnumeric():
			messagebox.showerror("Invalid Value for 'Number of Locations'", "You entered a non-number value for'Number of Locations'\nPlease enter a number value for the feild\ne.g '5', '10', '37', etc.")
			return

		if not self.minutes_per.get().isnumeric():
			messagebox.showerror("Invalid Value for 'Minutes per location'", "You entered a non-number value for'Minutes per loation'\nPlease enter a number value for the feild\ne.g '5', '10', '37', etc.")
			return
		...
		print(f"{self.getControlLayer().getValue('column choice')}, {int(self.number_of_locations.get())}, {int(self.minutes_per.get())}")
